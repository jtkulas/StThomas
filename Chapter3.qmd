...
+ Project:
  + Confirm construct name
  + Share 2-3 empirical articles<br>(intragroup) 
  + Discuss construct <br>(intergroup)
...
library(ggplot2)
ggplot(iris)+
  geom_point(aes(Petal.Width,Sepal.Length, size=2), show.legend=FALSE)
...
#| fig-height: 8
ggplot(iris)+
  geom_point(aes(Petal.Length,Sepal.Length, size=2), show.legend=FALSE)
...
ggplot(iris)+
  geom_point(aes(Petal.Width,Petal.Length, size=2), show.legend=FALSE)
```
...
::: {.column width="70%"}
::: {.ninety}
+ $Z$ scores below distribution's [median]{.definition} always negative
